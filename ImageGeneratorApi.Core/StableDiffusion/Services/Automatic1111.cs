...
using ImageGeneratorApi.Core.StableDiffusion.Services.Outpaint;
    ...
    : IImageGenerator, IImageAnalyser, IImageUpscaler, IImageOutpainter
{
    ...
    public Automatic1111(StableConfiguration config, StableDiffusionBackendCache backends)
    {
        _backends = backends;
    ...
    public async Task<IReadOnlyCollection<SixLabors.ImageSharp.Image>> Text2Image(int? seed, Prompt prompt,
        Func<ProgressReport, Task>? progressReporter = null, int batch = 1)
    {
    ...
    public async Task<IReadOnlyCollection<SixLabors.ImageSharp.Image>> Image2Image(int? seed,
        SixLabors.ImageSharp.Image inputImage, Prompt prompt, Func<ProgressReport, Task>? progressReporter = null,
        int batch = 1)
    {
        // Get the backend and lock it for the duration of this operation
        using var scope = await (await GetBackend()).Lock(default);
        ...
        using SixLabors.ImageSharp.Image image = inputImage.CloneAs<Rgba32>();

        // Scale down to the correct width
        Size size = new Size((int)_width, 0);
        image.Mutate(a => a.Resize(size));
        ...
        return await result.Images
            .Take(result.Images.Count -
                  (cnetConfig == null
                      ? 0
                      : 1)) // Skip the last image if cnet is used, to remove the guidance image which is added to the end
            .ToAsyncEnumerable()
    ...
    public async Task<SixLabors.ImageSharp.Image> UpscaleImage(SixLabors.ImageSharp.Image inputImage, uint width, uint height,
    ...
    public async Task<IReadOnlyCollection<SixLabors.ImageSharp.Image>> Outpaint(SixLabors.ImageSharp.Image inputImage, string positive, string negative,
        Func<ProgressReport, Task>? progressReporter = null)
    {
        // Get the backend and lock it for the duration of this operation
        using var scope = await (await GetBackend()).Lock(default);
    ...
    private static async Task<T> PumpProgress<T>(IStableDiffusion backend, Task<T> task,
