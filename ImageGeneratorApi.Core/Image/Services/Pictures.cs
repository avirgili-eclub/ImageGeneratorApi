...
using ImageGeneratorApi.Core.StableDiffusion.Utilities;
    ...
    : MuteInteractionModuleBase
{
    ...
    public async Task Analyse(IAttachment attachment, InterrogateModel model = InterrogateModel.DeepDanbooru)
    {
        ...
        var image = _http.GetStreamAsync(attachment.Url);
        ...
        var description = await _analyser.GetImageDescription(image.Result, model);
    ...
    public async Task Metadata(IAttachment attachment)
    {
        ...
        var img = await SixLabors.ImageSharp.Image.IdentifyAsync(image.Result);
    ...
    public async Task MidjourneyButtonWithIndex(string buttonType, string buttonIndex)
    {
        try
        {
            ...
            await new SocketMessageComponentGenerationContext(config, _generator, _upscaler, _outpainter, _http, _storage).Run();
        }
        catch (Exception ex)
        {
